...
//  primaryColorBrightness: Brightness.light,
  ...
  iconTheme: IconThemeData(
      color: Colors.white// icones do body
  ),

  accentIconTheme: IconThemeData(
      color: Colors.white// icones flutuantes
  ),
...
//          : kIOSTheme,
  ...
  Widget _autenticarCliente(BuildContext context){
    if(Session.getCliente() != null || c != null){
      ...
      print('Session.getCliente().codigoIndicacao: ${Session.getCliente().codigoIndicacao}');
      ...
      if(Session.getCliente().id == 1070){
        ...
        if(Session.getCliente().token != token){
          ...
          ClienteDao.internal().updateToken(Session.getCliente().id, token).then((client){
            print("client: ${client}");
            if(client != null){
...
//          });
        ...
        if((Session.getCliente().login == null || Session.getCliente().login.isEmpty)
            || (Session.getCliente().senha == null || Session.getCliente().senha.isEmpty)){
        ...
        child:  Center(
          child: Column(
            ...
            children: <Widget>[
              ...
              Text('Para prosseguir, é necessário atualizar algumas informações sobre seu cadastro nos nossos servidores',
                style: TextStyle(
                    fontSize: 16.0
                ),
              ...
              FlatButton(
                ...
                onPressed: () {
                  Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (context){
                    return CadastrarClientePage(Session.getToken(), numero: widget.cliente.telefone, cliente: widget.cliente);
                  }), (routes) => false,
                  );
                },
              )
