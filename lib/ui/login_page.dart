...
import 'endereco_confirm_page.dart';
      ...
      body:
          Container(
            decoration: BoxDecoration(
              ...
              child: Stack(
                children: <Widget>[
                  Container(color: Theme.of(context).accentColor.withOpacity(0.6),),
                  Container(
                        ...
                        children: <Widget>[
                          ...
                          Container(
                            ...
                            TextField(
                              ...
                              decoration: InputDecoration(
                                contentPadding: EdgeInsets.only(left: 10.0, right: 10.0, top: 4.0, bottom: 25.0),
                          ...
                          Container(
                            ...
                            child: RaisedButton(
                                child: Text('Acessar',
                                  style: TextStyle(
                                    color: active ? Theme.of(context).backgroundColor : Colors.grey[600],
                                    fontSize: 15.0
                                  ),
                                ),
...
//                              :
                          Container(
                                  ...
                                  child: FlatButton(
                                    ...
                                    onPressed: active ? () async {
//                                      JsonUtils.buscarCliente(telefone: _numberVisitant).then((cliente){
//                                        if(cliente != null){
...
//                                      });
                                      ...
                                      ClienteDao.internal().getClientFromPhone(telefone: _numberVisitant).then((response){
...
//      _verificarCliente();
    ...
    final PhoneCodeAutoRetrievalTimeout codeAutoRetrievalTimeout = (String verificationId) {
          ...
          MessageUtil.alertMessageScreen(context,
              ...
              'O tempo limite de recuperação automática do código sms acabou. Tente novamente, ou faça login utilizando seu e-mail.',
              [
                FlatButton(
                  child: Text('OK'),
                  onPressed: () {
    ...
    return
      showDialog(context: context,
        ...
        builder: (BuildContext context){
          return AlertDialog(
            title: Text('Insira o código SMS recebido'),
            content: Container(
              height: 85.0,
              child: Column(
                  children: [
...
//                signIn();
    ...
    try{
      ...
      switch(exception.code){
        case 'ERROR_INVALID_VERIFICATION_CODE':
          ...
          subtitle = 'O código de verificação de sms usado para criar a credencial de autenticação do telefone é inválido. Reenvie o código de verificação sms e use o código de verificação fornecido pelo usuário';
          break;
      }
  ...
  void _verificationFailed(AuthException authException){
    print('Verificação falhou. Code: ${authException.code}. Message: ${authException.message}');

    switch(authException.code){
      case 'quotaExceeded':
        _messageError(
            titleError: 'SEU NÚMERO FOI BLOQUEADO!',
            subtitleError: 'Bloqueamos todos os pedidos deste dispositivo devido a atividades incomuns. Tente novamente mais tarde.'
        );
        break;
      case 'invalidCredential':
        _messageError(
            titleError: 'O FORMATO DO NÚMERO DE TELEFONE FORNECIDO ESTÁ INCORRETO!',
            subtitleError: 'Digite o número do telefone em um formato que possa ser analisado no formato E.164 - '
                '(xx) 9xxxx-xxxx.'
        );
        break;
    }
...
//    JsonUtils.buscarCliente(telefone: phoneNumber).then((cliente){
      ClienteDao.internal().getClientFromPhone(telefone: phoneNumber).then((response){
      print("response: ${response}");
      if(response['status'] == 'success'){
        Cliente cliente = Cliente.fromJson(response['cliente']);
        ...
        Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (context){
          return CadastrarClientePage(Session.getToken(), numero: phoneNumber);
        }), (routes) => false,
        );
      }
    });
  ...
  void _verificarCadastro(Cliente cliente){
    if((cliente.login == null || cliente.login.isEmpty)
      ...
      MessageUtil.alertMessageScreen(context,
          ...
          [
            FlatButton(
              ...
              onPressed: () {
                Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (context){
                  return CadastrarClientePage(Session.getToken(), numero: cliente.telefone, cliente: cliente);
                }), (routes) => false,
                );
              },
            )
      ...
      print('LoginPage._verificarCadastro =============================================================');
