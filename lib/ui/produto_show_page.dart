...
class _ProdutoShowPageState extends State<ProdutoShowPage> {
  ...
  String _verificarMultiplos(AsyncSnapshot snapshot, int index) {
    if (snapshot.data[index].isMultiple) {
      return "No minimo: ${snapshot.data[index].minimo} No máximo: ${snapshot
          .data[index].maximo}";
    } else {
                ...
                child: Column(
                  ...
                  children: <Widget>[
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                            image: DecorationImage(
                                image: ImageUtil.loadWithRetry('${_produto.imagem}'),
                                fit: BoxFit.cover)),
                      ),
                    ),
                    Padding(
                    ...
                    Padding(
                      ...
                      child: Text(_produto.promocao == true
                          ? "De: R\$ ${_produto.preco}"
                          : '',
                          style: TextStyle(
                              fontSize: 15.0)),
...
//                              color: Color.fromRGBO(36, 36, 143, 1.0),
                              child: Text(Session.getCliente().id == 1070 ? 'Login': "Adicionar ao carrinho",
                                  style: TextStyle(
                                      color: Colors.white,
                                      fontWeight: FontWeight.bold,
                                      fontSize: 14.0)),
...
//                              ),
                              ...
                              onPressed: Session.getCliente().id == 1070
                                  ? ()async{
                                    ...
                                    await MessageUtil.alertMessageScreen(context,
                                        'Valide seus dados',
...
//                  color: Colors.white,
  ...
  Widget _screenListAdicionais(BuildContext context, AsyncSnapshot snapshot, int index) {
...
//                    snapshot.data[index].adicionais[indice].selected = 1;
                    ...
                    for(Adicional adicional in variacao.adicionais){
                      if(adicional.id == snapshot.data[index].adicionais[indice].id){
...
//                    print(snapshot.data[index].adicionais[indice].selected);
                ...
                secondary: Text(
                  "R\$ ${snapshot.data[index].adicionais[indice].valor}".replaceAll('.', ','),
                  style: TextStyle(
                      color: Colors.green,
                      fontWeight: FontWeight.bold,
                      fontSize: 14.0),
        ...
        trailing: Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Text("Selecionar",
                  style: TextStyle(fontSize: 12.0, color: Configuration.colorDefault4))
            ],
          ),
        ),
        onTap: () async {
          ...
          if (variacoesEscolhidas.isNotEmpty) {
            Variacao variacaoAuxiliar = _isListVariacao(
    ...
    if (variacoesEscolhidas.isEmpty) {
      ...
      if (variacaoNew != null) {
        for (Adicional adicionalAuxiliar in variacaoNew.adicionais) {
          if (variacaoNew.adicionais.elementAt(variacaoNew.adicionais.length - 1) ==
              adicionalAuxiliar) {
...
//        print(adicional.descricao);
        if(adicional.selected){
          lista.add(AdicionalEscolhido(adicional.id, adicional.descricao, variacao.descricao, adicional.valor));
        }
      }
    }
...
//          print('entrou aqui');
          ...
          MessageUtil.alertMessageScreen
            (context, "Você precisa selecionar, pelo menos ${variacaoVerified.minimo} ${variacaoVerified.descricao.toLowerCase()}", "", actions);
          ...
          double valorItem = (_produto.promocao
              ...
              : double.parse(_produto.valorProduto)) +_calcularValorProdutos(adicionais);
