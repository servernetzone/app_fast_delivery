...
class ParceiroDao{
  ...
  Future<List<Parceiro>> list(int idCidade) async {
    var data = await http.get((Factory.internal().getUrl()+'cidade/$idCidade/parceiros/'), headers: {'Accept': 'application/json'});
  ...
  Future<List<Parceiro>> listSeguimentos(int idCidade, String seguimento) async {
    var data = await http.get((Factory.internal().getUrl()+"cidade/$idCidade/parceiros/$seguimento/"), headers: {'Accept': 'application/json'});
    var jsonDataList = json.decode(utf8.decode(data.bodyBytes));
    List<Parceiro> parceiros = [];
    ...
    for (var p in jsonDataList) {
      ...
      Parceiro parceiro = Parceiro(p['pk'], p['nome'], p['razaoSocial'], p['imagemLogo'], p['imagemBackground'],
          situacao, p['getestimativaEntrega'], p['getValoresEntrega'],p['seguimento'], p['descricao'],  p['proxAbertura']
          , p['proxFechamento'],  p['aceitaCartao'], p['resultado_avaliacao'] + 0.0, p['porcentagemCartao'] == null ? 0.0 : p['porcentagemCartao']
          , p['permitir_retirada_estabelecimento']
      );
      parceiros.add(parceiro);
    }
